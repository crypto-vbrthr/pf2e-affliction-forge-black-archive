import {
  BLACK_ARCHIVE_CONTENT_VERSION,
  BLACK_ARCHIVE_DEFINITIONS,
  BLACK_ARCHIVE_MODULE_ID
} from "./content.js";

const MODULE_ID = BLACK_ARCHIVE_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "black-archive";
const LIBRARY_ID = "black-archive.core";
const PACK_NAME = "affliction-forge-black-archive";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_BA.Module.Library", "The Black Archive"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: BLACK_ARCHIVE_CONTENT_VERSION,
...
async function syncManagedPack(api, pack) {
  ...
  const creates = [];
  const updates = [];
  for (const definition of BLACK_ARCHIVE_DEFINITIONS) {
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_BA.Module.Title", "PF2E Affliction Forge: The Black Archive"),
    moduleId: MODULE_ID,
    version: BLACK_ARCHIVE_CONTENT_VERSION,
    metadata: {
      themes: ["prestige", "experimental", "affliction"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_BA.Module.LibraryDescription", "Rare experimental afflictions with semantic Creature Forge tags and advanced runtime mechanics."),
      metadata: {
        themes: ["prestige", "experimental", "affliction"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: BLACK_ARCHIVE_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: BLACK_ARCHIVE_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_BA.Module.SyncFailed", "The Black Archive could not synchronize its library. See console.")
    );
  });
});
