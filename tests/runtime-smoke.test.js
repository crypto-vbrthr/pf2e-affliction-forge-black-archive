...
const ADDON = "pf2e-affliction-forge-black-archive";
const COLLECTION = "world.affliction-forge-black-archive";
...
test("startup provisions, synchronizes, and registers the provider library", async () => {
  ...
  await readyHandler(api);
  await new Promise((resolve) => setTimeout(resolve, 10));
  assert.equal(index.length, 24, "second sync updates in place and removes stale managed content");
